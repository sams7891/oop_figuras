...
public class OIzveide {
	...
	static int triIzvele() {
		if(Figuras.trijsturaObjekti.size() < 1)
			return -1;
		
		int izvele = MinkuTante.skaitlaParbaude(IzveidotieObjekti.izvaditTrijsturi(Figuras.trijsturaObjekti), 0, Figuras.trijsturaObjekti.size() - 1);
	...
	public static void izveidotObjektu() {
		...
		case 0:
			...
			if ((x == -1 || y == -1) && MinkuTante.ievade == null) {
				JOptionPane.showMessageDialog(null, "Centra punkts netika izveidots!", "Paziņojums", JOptionPane.QUESTION_MESSAGE);
				break;
			}
		...
		case 1:
			...
			case "Centra punkts un malas":
				...
				if(p == -1 || a == -1) {
					JOptionPane.showMessageDialog(null, "Četrstūris netika izveidots!", "Paziņojums", JOptionPane.ERROR_MESSAGE);
					break;
				}
